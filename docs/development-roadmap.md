...
## 1. Reality Check
- Repo currently contains documentation only (`docs/system-specs.md`, `AGENTS.md`, `CLAUDE.md`, minimal README).
- No source code, configs, Prefect flows, or infra scripts exist yet.
- Architecture mandates two-node deployment (controller + GPU), Prefect orchestration, NFS storage, Telegram alerts, and yt-dlp ingestion.
...
| **Phase 1 – Infrastructure Foundation** | Prefect server + storage + monitoring skeleton | Implement Docker Compose for Prefect Server + Postgres on `etl-node-01`; configure GPU work pool w/ concurrency limit; set up NFS export & mount scripts; stub Grafana/Loki/Prometheus stack. |
| **Phase 2 – Agent Implementations** | Build Watcher → Ear → Brain → Courier services | 1) Watcher (RSS + yt-dlp + ledger). 2) Ear (FastAPI + faster-whisper). 3) Brain (Ollama client + prompts). 4) Courier (Markdown + Telegram). |
...
| **Phase 4 – QA & Ops Readiness** | Tests, docs, monitoring | Unit/integration tests (CPU mocks via Docker Compose), README quickstart, dashboards, runbooks for WOL/NFS/yt-dlp issues. |
...
3. **Prefect & Storage Prep**
   - Draft Docker Compose (Prefect Server + Postgres + Loki seed).
   - Write NFS export/mount instructions + rsync cron template.
4. **Watcher Prototype**
   - Implement CLI or Prefect task for RSS parsing + yt-dlp download (with documented flags).
   - Persist metadata JSON + ledger entry; test against target channels (All-In, Lex Fridman).

## 4. Prefect & Home-Lab Considerations
...
| **5** | Prefect flows, retry/backoff logic, dead-letter handling, Wake-on-LAN automation |
| **6** | Monitoring stack, integration tests, README + runbooks, launch readiness review |
