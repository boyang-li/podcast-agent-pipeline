...
| **Phase 1 – Infrastructure Foundation** | Prefect server + storage + monitoring skeleton | Implement Docker Compose stacks (local/staging/prod); configure GPU work pool w/ concurrency limit; set up NFS export & mount scripts; stub Grafana/Loki/Prometheus stack. |
...
| **Phase 4 – QA, Staging Automation & Ops Readiness** | Automated staging tests + monitoring | Container health checks, Prefect staging runs, README quickstart, dashboards, runbooks for WOL/NFS/yt-dlp issues. |
...
3. **Prefect & Storage Prep** (✅)
   - Docker Compose stacks for local/staging/prod committed under `docker/`.
...
- Local dev uses CPU mocks inside Docker; GPU-only validation happens in staging (automated by CI).
- CI builds multi-arch images via docker buildx; staging/prod compose stacks pull from GHCR/local registry.
...
| **6** | Monitoring stack, staging integration tests, README + runbooks, launch readiness review |
